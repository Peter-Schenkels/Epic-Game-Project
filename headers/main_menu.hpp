...
public:
		...
		window(window) 
	{
		...
		if (!main_menu_music.openFromFile("Paper Boats.wav")) {
			std::cout << "Main menu theme not loaded\n";
		}
		main_menu_music.setVolume(10.f);
	...
	int Run() {
		...
		if (!Texture.loadFromFile("img/hentai_1_kopie.jpg"))
		{
		...
		Menu1.setPosition({ 875.f, 400.f });

		Menu2.setFont(Font);
		Menu2.setCharacterSize(50);
		Menu2.setString("Exit");
		Menu2.setPosition({ 875.f, 500.f });

		Menu3.setFont(Font);
		Menu3.setCharacterSize(50);
		Menu3.setString("Continue");
		Menu3.setPosition({ 875.f, 400.f });
